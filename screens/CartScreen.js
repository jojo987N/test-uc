...
import { language, currency, APP_CONSTANT } from '../global'
        ...
        .map(([restaurantName, quantity], index) => (
          ...
          >
            ...
            <View style={styles.name_quantity_price}>
              ...
              <Text style={styles.quantity_price}>{quantity} {APP_CONSTANT.TEXT.ARTICLE}{quantity > 1 ? "s" : ""} • {items.reduce((a, v) => v.restaurantName === restaurantName ? a + v.price : a, 0).toLocaleString(language, { style: "currency", currency: currency })}</Text>
              <Text style={styles.address}>{APP_CONSTANT.TEXT.DELIVER_TO}{address.description}</Text>
            </View>
            <AntDesign name="right" size={20} color={colors.black} style={styles.icon} />
          </TouchableOpacity>
        ))}
    </View>
  );
