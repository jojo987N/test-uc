  ...
  useEffect(() => {
    if (status === "Driver is on the way for pickup")
      getDriverInfos(
        ...
        console.log(
          lat + (driverLat - lat) * 0.5,
          lng + (driverLng - lng) * 0.5,
          "fff"
        );
      });
  }, [status]);
  return (
    <View style={{}}>
      ...
      >
        ...
        {driverLat && driverLng && !local ? (
          <DisplayMapviewDirections
            apikey={apikey}
            toLat={lat}
            toLng={lng}
            fromLat={driverLat}
...
export const DisplayMapviewDirections = ({
  fromLat,
  fromLng,
  toLat,
  toLng,
...
const styles = StyleSheet.create({
  ...
  timeContainer: { padding: 10, borderRadius: 20, backgroundColor: "white" },
