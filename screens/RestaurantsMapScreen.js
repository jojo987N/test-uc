...
MapboxGL.setAccessToken(
  "pk.eyJ1IjoiY2FibGl2ZWVyIiwiYSI6ImNsY3lqbHFxNzBuN3czcHFsb3NjM245c24ifQ.wNFmAbLxPiik8S01gAru3Q"
);
...
export default function RestaurantsMapScreen({ route, navigation }) {
  ...
  const restaurantDataSort = restaurantData.filter(
    (c) =>
  ...
  const [focus, setFocus] = useState(
    new Array(restaurantData.length).fill({
    ...
    >
      ...
      >
        <MapboxGL.Camera
          ref={camera}
          defaultSettings={{
            centerCoordinate: [-122.429213, 37.769535],
            //zoomLevel: 17.4,
            zoomLevel: 11.4,
          }}
        />

        {/* <MapboxGL.PointAnnotation coordinate={[-122.429213, 37.769535]} /> */}

        {/* <MapboxGL.MarkerView id={"marker"} coordinate={[-122.429213, 37.769535]}>
          <View>
        ...
        <RestaurantMarkersMapbox
          restaurantDataSort={restaurantDataSort}
          focus={focus}
        ...
        onScroll={
          horizontal
            ? (event) => {
                ...
                //   centerCoordinate: [restaurantDataSort[Math.round(x / w)].coordinates.longitude, restaurantDataSort[Math.round(x / w)].coordinates.latitude],
                // });

                camera.current.flyTo(
                  [
                    restaurantDataSort[Math.round(x / w)].coordinates.longitude,
                    restaurantDataSort[Math.round(x / w)].coordinates.latitude,
                  ],
                  12000
                );
                //camera.current.zoomTo(16, 1000);
            ...
            : (event) => {
                ...
                setDirection(
                  event.nativeEvent.contentOffset.y > offset ? "up" : "down"
                );
      ...
      >
        ...
        <View
          style={{
            ...
            backgroundColor: focus[index].backgroundColor,
            zIndex: focus[index].zIndex,
          }}
        >
          <MaterialIcons
          ...
          <View>
            <View style={styles.markerContainer}>
              ...
              <View
                style={{
                  ...styles.restaurant_marker,
                  ...styles.restaurant_marker_shadow,
                }}
              >
                <TouchableOpacity
                  onPress={() => {
