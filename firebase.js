...
export const getDriverInfos = async (
  setDriverName,
  setCar,
  setDriverImage,
  bottomSheet,
    ...
    snapshot.docs.forEach((doc) => {
      if (
        (doc.data().createdAt &&
          ...
          doc.data().User.id === auth.currentUser?.uid,
        doc.data().driverId)
      ) {
        ...
        driverInfos(doc.data().driverId).then((snapshot) =>
          snapshot.docs.forEach((doc) => {
...
export const addRestaurants = (restaurants) => {
  restaurants.forEach((restaurant) => {
    addDoc(restaurantsCol, restaurant).then(() => console.log("ajouté"));
  });
};
const foodsCol = collection(db, "foods");
const addfoods = () => {
  getDocs(restaurantsCol).then((snapshot) =>
    snapshot.docs.forEach((doc) => {
...
const getFoodsAll = () => {
  getDocs(foodsCol).then((snapshot) => {
...
export const getCategoriesFromRestaurant = async (restaurantId) => {
  ...
  const categoriesRestaurantsResult = categoriesRestaurants.filter(
    (categoryRestaurant) => categoryRestaurant.restaurantId === restaurantId
  );
  const categories = await getCategories();
  return categoriesRestaurantsResult.map((categoryRestaurantResult) =>
    categories.find(
      (category) => category.id === categoryRestaurantResult.categoryId
    )
  );
};
export const searchRestaurantsByCategory = async (categoryId) => {
  const categoriesRestaurants = await getCategoriesRestaurants();
  const categoriesRestaurantsResult = categoriesRestaurants.filter(
    (categoryRestaurant) => categoryRestaurant.categoryId === categoryId
  );
  const restaurants = await getRestaurantsFromFirebase();
  return categoriesRestaurantsResult.map((categoryRestaurantResult) =>
    restaurants.find(
      (restaurant) =>
        restaurant.restaurantId === categoryRestaurantResult.restaurantId
    )
  );
};
